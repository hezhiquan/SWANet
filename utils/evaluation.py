...
from .plt_utils import tensor2img
from .common import paths_from_folder
...
class Measure:
    ...
    def test_dir(self, low_path, high_path):
        ...
        assert len(low_files) > 0 and len(low_files) == len(high_files), "文件数目为0或不相等"
        self.reset()
    ...
    # def get_avg_str(self, num=(3, 4)):
    #     avg_result = self.get_avg()
    #     return "avg:" + " ".join(
    #         ["{0}:{1:.{2}f}".format(self.metric_names[i], avg_result[i], num[i]) for i in range(len(avg_result))])
...
def measure_dirs(low_path, high_path, metric_type="fr", use_gpu=True, verbose=True):
    ...
    if metric_type == "fr":
        ...
        measure = Measure(use_gpu=use_gpu, metric_type=["psnr", "ssim", "lpips"])
        ...
        for pathA, pathB in tqdm(list(zip(paths_a, paths_b))):
            ...
            # result = OrderedDict()
            t = time.time()
            measure.measure_from_path(pathA, pathB, add_record=True)
            # result['psnr'], result['ssim'], result['lpips'] = measure.measure_from_path(pathA, pathB, add_record=True)
            d = time.time() - t
            # vprint(f"{pathA.split('/')[-1]}, {pathB.split('/')[-1]}, {format_result(**result)}, {d:0.1f}")

            # results.append(result)

        # psnr_mean = np.mean([result['psnr'] for result in results])
        # ssim_mean = np.mean([result['ssim'] for result in results])
        # lpips_mean = np.mean([result['lpips'] for result in results])

        vprint(f"Final Result: {format_result(*measure.get_avg())}, {time.time() - t_init:0.1f}s")
...
if __name__ == "__main__":
    ...
    if len(dirA) > 0 and len(dirB) > 0:
        measure_dirs(dirA, dirB, use_gpu=args.use_gpu, metric_type=args.metric_type, verbose=True)
